...
9. Read the project instructions from the project root if they exist. Hold those standards as context and ensure
...
- Code intelligence checks are advisory. They surface issues early but do not block execution unless the user
  chooses to stop. Test runs are not advisory: a failing suite blocks COMPLETED.
...
- The project instructions are applied during execution. If any step introduces patterns that should be
  documented there, flag this to the user immediately.
- Execution state is best-effort. If the session ends mid-step, the state file may not reflect the
