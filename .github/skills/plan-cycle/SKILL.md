...
name: plan-cycle
description: >-
  Draft, critique and merge a plan in one loop, then stop for approval. Use when the user runs /plan-cycle or
  asks for the full plan, critique and merge loop in a single pass.
license: MIT
...
4. Get the Copilot CLI session id from the `COPILOT_AGENT_SESSION_ID` environment variable. Read it with
...
5. Clean up stale sessions: scan `[plansFolder]/.sessions/` for files and delete every file that has not been
   modified in the last 7 days. This is non-blocking cleanup, never abort the run because of it.
6. Read the current session's plan from `[plansFolder]/.sessions/[sessionId]` if it exists. Store as `sessionPlan`.
...
8. Select the plan for this cycle:
   - If the user passed a name in the prompt that invoked this skill and it is a slug that matches an existing
     plan folder, select that plan.
   - Else if the user passed a name in the prompt, treat it as the name of a new plan and create it by following
     the plan naming and folder creation steps of [../plan-create/SKILL.md](../plan-create/SKILL.md): the slug
     rules, the name collision check, the folder creation and the `plan.md` template from
...
9. Update the session file `[plansFolder]/.sessions/[sessionId]` with the selected plan slug (create if needed).
...
11. Prepare the draft. Tell the user "plan.md is still the template. Drafting a first pass." and get the goal:
    - Use the text the user passed in the prompt as the goal when it describes what to build.
...
12. Research the codebase, then write the draft:
    - Read the project instructions and `README.md`, and the existing files the goal touches.
...
13. Check prerequisites once, before the loop:
    ...
    - If no project instructions file exists: respond with "Create a `.github/copilot-instructions.md` file in
      the root of your project." and stop.
    - Detect the project languages and probe the language server as described in the language server
      availability step of [../plan-critique/SKILL.md](../plan-critique/SKILL.md). Record whether a language
      server is available, and reuse that answer for every iteration instead of probing again.
...
- Each iteration is a full adversarial pass over the plan and the codebase, so a three iteration run on a large
  plan is expensive. Set `cycleIterations` to 1 or 2 in `.copilot/plan-critique-config.json` for small plans.
